        ...
        if(seriesCount > 0){
            for(i = 0; i < seriesCount; i++){
                ...
                for(var j = 0; j < data.length; j++){
                	var dd = data[j];
                	if(!dd[currentSeries.yField] || dd[currentSeries.yField]==null)
		            {
//		            	if(j>0)
//		            		dd['31'] = data[j-1]['31'];
		            }
	...
	setValue: function(values){
		if(values != null)
		{
			...
			if(values.list)
			{
				...
				{
					var match = this.store.find(this.xField, values.list[i][this.xField]);
					if(match != -1)
					{
						var value = values.list[i];
						var ovalue = this.store.getAt(match);
						Ext.apply(ovalue.data, value);
			...
			this.store.loadData(values, false);
		}
	},
...
// some extensions
Ec.CartesianChart = Ext.extend(Ext.chart.CartesianChart, {
    mode           : 'local',
	...
	initComponent  : function(){
        Ec.CartesianChart.superclass.initComponent.call(this);
    	this.addEvents(
            'refreshvalue'
        );
        this.store = new Ext.data.Store({
...
//             ,Ext.data.Record.create(this.fields)
         ...
         if(this.updatefrequent)
         {
         	this.updatetimer = Ext.TaskMgr.start({
	            ...
	            run: function() {
	            	var options = {};
//	            	this.getReference(this.store,options);
	            	var count = this.store.getCount();
	            	if(count>0)
	            	{
	            		var lastdata = this.store.getAt(count-1).data;
	            		options['lastx'] = lastdata[this.xField];
	            		lastdata = this.store.getAt(0).data;
	            		options['startx'] = lastdata[this.xField];
	            	}
	            	if(this.seriesparam)
	            		options['updateseries'] = this.seriesparam;
    ...
    resumeUpdate : function(){
    	if(this.updatetimer)
    		Ext.TaskMgr.start(this.updatetimer);
    },
    onMetaChange  : function(store, metaData){
    	var series = metaData.extData;
    	this.setUpdateSeries(false);
	...
	setValue: function(values){
		if(values != null)
		{
			...
			this.store.loadData(values, this.updatefrequent?true:false);
			if(this.updatefrequent && this.updatecount)
			{
	        	if(this.store.getCount() > this.updatecount)
	        	{
	        		var flen = values.totalCount;
	        		this.store.remove(this.store.getAt(0,flen-1));
	        	}
			}
		}
	},
	setUpdateSeries: function(update){
		this.seriesparam = update;
	}
});
Ext.reg('ec_serieschar', Ec.CartesianChart);

Ec.HandlerManager.regEventHandler('ec_serieschar', 'refreshvalue', Ec.EventHandler.FrequencyRefresh);	

Ec.EventHandler.ItemClick = function(chart, value) {
	value = value || {};
	value.action =  'itemchange';
	Ec.Request.fireFieldEvent(this, value);
}
Ec.HandlerManager.regEventHandler('ec_serieschar', 'itemchange', Ec.EventHandler.ItemClick);
