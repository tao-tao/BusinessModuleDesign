...
public class BoModelFactory  extends AbstractModelFactory implements IModelCreator{
	
	public static String TYPE = "business_object";

	public BoModelFactory() {
		setPriority(17000);
		this.setType(TYPE);
	}
	...
	public class ModelParser extends AbstractModelFactory.ModelParser {
		...
		public Object createNewNode(IResourceDelegate ifiledelegate) throws ModelParseException {
			if (ifiledelegate instanceof IFolderDelegate){
				...
				return new BoNode(new EclipseFolderDelegate(resource));
			}
			return null;
		}
...
//		{
		...
		try {
			...
			try {
				...
				domainFolder = folder.getFolder(new Path("dto"));
				...
				if(!folder.exists())
					folder.create(true, true, null);
//				folder = folder.getFolder(new Path("domain"));
//				if(!folder.exists())
//					folder.create(true, true, null);
