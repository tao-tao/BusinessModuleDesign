...
public class LogicModelFactory  extends AbstractModelFactory implements IModelCreator{
	
	public static String TYPE = "logic";

	public LogicModelFactory() {
		setPriority(17000);
		this.setType(TYPE);
	}
...
//		return false;
	...
	public class ModelParser extends AbstractModelFactory.ModelParser {
		...
		public Object createNewNode(IResourceDelegate ifiledelegate) throws ModelParseException {
			if (ifiledelegate instanceof IFolderDelegate){
				...
				return new LogicNode(new EclipseFolderDelegate(resource));
			}
			return null;
		}
	...
	private List copyListExcludeLastOne(List<?> srcList){
	...
	private IFolder getConfigFolder(IResourceDelegate ifiledelegate){
		...
		IFolder folder = null;
		IFolder logicFolder = null;
		try {
			folder = (IFolder)ifiledelegate.getProject().getFolder(path).getResource();
			try {
				if(!folder.exists())
					folder.create(true, true, null);
				logicFolder = folder.getFolder(new Path("service"));
				if(!logicFolder.exists()){
					logicFolder.create(true, true, null);
				}
