...
public class BusinessModelFactory extends AbstractModelFactory implements IModelCreator{
	...
	public class ModelParser extends AbstractModelFactory.ModelParser {
		...
		public Object createNewNode(IResourceDelegate ifiledelegate) throws ModelParseException {
			if (ifiledelegate instanceof IFolderDelegate){
				...
				return new BusinessNode(new EclipseFolderDelegate(resource));
			}
...
//				path = path.substring(0, i);
				...
				try {
					...
					try {
						...
						folder = folder.getFolder(new Path("business"));
						if(!folder.exists())
							folder.create(true, true, null);
		...
		public Object[] getChildren(INode object) {
			...
			FileUtil.listFiles((IFolder) node.getResource().getResource(), presFiles, "", new String[]{".fcd"}, null);
			PackageNode pn = null;
			...
			if (presFiles.size() > 0) {
				...
				if (fs.size() > 0) {
					for (int j = 0; j < fs.size(); j++) {
						...
						INode p = null;
						p = new BusinessUseCaseNode(new EclipseFileDelegate(file));
						p.setParent(node);
						childList.add(p);
					}
				}
	...
	public AbstractNode getOrCreateNode(IFileDelegate file, boolean forceCreate) {
		...
		if(sourceFolder == null)
			return null;
		INode model = (INode) ModelManagerImpl.getInstance().getPool().getModel(sourceFolder, BusinessModelFactory.TYPE, true, null);
		if(model != null)
		{
