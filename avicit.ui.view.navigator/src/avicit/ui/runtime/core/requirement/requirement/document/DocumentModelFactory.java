...
public class DocumentModelFactory extends AbstractModelFactory implements IModelCreator{
	...
	public class ModelParser extends AbstractModelFactory.ModelParser {
		...
		public Object createNewNode(IResourceDelegate ifiledelegate) throws ModelParseException {
			if (ifiledelegate instanceof IFolderDelegate){
				...
				return new DocumentNode(new EclipseFolderDelegate(resource));
			}
			...
			String path = ifiledelegate.getProjectRelativePath();
				...
				try {
					...
					try {
						...
						folder = folder.getFolder(new Path("document"));
						if(!folder.exists())
							folder.create(true, true, null);
...
//			}
	...
	public AbstractNode getOrCreateNode(IFileDelegate file, boolean forceCreate) {
		...
		if(sourceFolder == null)
			return null;
		INode model = (INode) ModelManagerImpl.getInstance().getPool().getModel(sourceFolder, DocumentModelFactory.TYPE, true, null);
		if(model != null)
		{
