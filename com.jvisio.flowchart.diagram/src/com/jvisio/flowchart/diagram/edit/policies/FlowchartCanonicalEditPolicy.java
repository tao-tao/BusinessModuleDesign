...
public class FlowchartCanonicalEditPolicy extends CanonicalConnectionEditPolicy {
	...
	protected List getSemanticChildrenList() {
		...
		EObject nextValue;
		int nodeVID;
		for (Iterator values = ((Flowchart) modelObject).getElements()
				.iterator(); values.hasNext();) {
			nextValue = (EObject) values.next();
			nodeVID = FlowchartVisualIDRegistry.getNodeVisualID(viewObject,
					nextValue);
			switch (nodeVID) {
	...
	private void collectAllLinks(View view) {
		...
		int diagramElementVisualID = FlowchartVisualIDRegistry
				.getVisualID(view);
	...
	private void storeTypeModelFacetLinks(EObject container,
		...
		if (FlowchartPackage.eINSTANCE.getFlowchart().isSuperTypeOf(
				containerMetaclass)) {
			for (Iterator values = ((Flowchart) container).getTransitions()
					.iterator(); values.hasNext();) {
				EObject nextValue = ((EObject) values.next());
				int linkVID = FlowchartVisualIDRegistry
						.getLinkWithClassVisualID(nextValue);
				if (TransitionEditPart.VISUAL_ID == linkVID) {
					Object structuralFeatureResult = ((Transition) nextValue)
							.getTarget();
					if (structuralFeatureResult instanceof EObject) {
						EObject dst = (EObject) structuralFeatureResult;
						structuralFeatureResult = ((Transition) nextValue)
								.getSource();
						if (structuralFeatureResult instanceof EObject) {
