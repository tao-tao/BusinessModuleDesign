 ...
 * 
 */
function DemoBusinessTrip(datagrid,url,searchD,form){
...
//初始化操作
DemoBusinessTrip.prototype.init=function(){
	var _self = this;
	...
	this._datagrid=$(this._datagridId).datagrid({
		url:this.getUrl()+"getDemoBusinessTripsByPage.json"
		});
}
//添加页面
DemoBusinessTrip.prototype.insert=function(){
	this.nData = new CommonDialog("insert","600","400",this.getUrl()+'Add/null',"添加",false,true,false);
	this.nData.show();
}
//修改页面
DemoBusinessTrip.prototype.modify=function(){
...
//详细页
DemoBusinessTrip.prototype.detail=function(id){
	this.nData = new CommonDialog("edit","600","400",this.getUrl()+'Detail/'+id,"详情",false,true,false);
	this.nData.show();
}
//保存功能
DemoBusinessTrip.prototype.save=function(form,id){
	var _self = this;
	$.ajax({
...
//删除
DemoBusinessTrip.prototype.del=function(){
	var rows = this._datagrid.datagrid('getChecked');
	var _self = this;
	var ids = [];
	var l =rows.length;
...
//重载数据
DemoBusinessTrip.prototype.reLoad=function(){
	this._datagrid.datagrid('load',{});
}
//关闭对话框
DemoBusinessTrip.prototype.closeDialog=function(id){
	$(id).dialog('close');
}

//打开查询框
DemoBusinessTrip.prototype.openSearchForm =function(){
	this.searchDialog.dialog('open',true);
}
//去后台查询
DemoBusinessTrip.prototype.searchData =function(){
...
//隐藏查询框
DemoBusinessTrip.prototype.hideSearchForm =function(){
	this.searchDialog.dialog('close',true);
}
/*清空查询条件*/
DemoBusinessTrip.prototype.clearData =function(){
	this.searchForm.find('input').val('');
	this.searchData();
};
DemoBusinessTrip.prototype.formate=function(value){
	if(value){
