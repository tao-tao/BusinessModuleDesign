...
function searchFun(){
    ...
    queryParams.filter_LIKE_syslogIp = $('#syslogIp').attr("value");
...
function exportServerData(){
	$.messager.confirm('确认','是否确认导出Excel文件?',function(r) {
        if (r) {
            ...
            var url = "platform/sysLog/sysLogController/exportServer";
			...
			<fieldset>
				...
				<table class="tableForm" id="searchForm" width='100%'>
					<tr>
						...
						<td colspan="2"><input name="syslogOp" id="syslogOp" class="easyui-combobox" style="width: 150px;" data-options="editable:false,panelHeight:'auto',valueField: 'label',textField: 'value',data: [{label: 'insert',value: '插入'},{label: 'delete',value: '删除'},{label: 'update',value: '修改'},{label: 'select',value: '查看'},{label: 'login',value: '登录'}]"/>
						</td>
						</td>
						<td>日志类型：</td>
						<td colspan="1"><input name="syslogType" id="syslogType" class="easyui-combobox" style="width: 150px;" data-options="
						valueField:'lookupCode',
						textField:'lookupName',
						editable:false,
						panelHeight:'auto',
						url:'platform/sysLog/sysLogController/initSysLogType.json'
						"/>
					...
					<tr>
						...
						<td>日志内容：</td>
						<td colspan="1"><input name="syslogTitle" id="syslogTitle"  editable="false" style="width: 150px;" />
						</td>
						<td>操作人IP：</td>
						<td colspan="2"><input name="syslogIp" id="syslogIp"  editable="false" style="width: 150px;" />
						</td>
	...
	<div data-options="region:'center',split:true,title:''" style="padding:0px;">	
		...
		<table id="sysLoglist" class="easyui-datagrid"
			data-options=" 
				...
				url: 'platform/sysLog/sysLogController/getSysLogDataByPage.json?',
				queryParams:{'':''}		
				">
			<thead>
				<tr>
