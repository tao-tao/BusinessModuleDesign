    ...
    $.widget("ui.portlet", {
        ...
        _createSinglePortlet: function(portlet, _ele, column, positon, pattrs) {
            ...
            var item = $('<div/>').addClass('ui-portlet-item ui-widget ui-widget-content ui-helper-clearfix ui-corner-all')
                        .data('option', pattrs);
            // dingrc 没有一个portlet时直接插入
            if(column[0].innerHTML === ''){
                ...
                item.appendTo(column);
	                ...
	                if (positon.x === 'last') {
	                	...
	                	//dingrc 选择某行插入时，如果此行没有portlet直接追加到末尾
	                	 if($(column).find('.ui-portlet-item').length<=positon.x){
	                		 item.insertAfter($(column).find('.ui-portlet-item:last'));
	                	 } else{
            ...
            //是否显示标题
            if(pattrs.isShowTitle!=null&&pattrs.isShowTitle=="1"){
                var title = $('<div/>', {
                ...
                // set icon for title
                if(pattrs.icon!=null&&pattrs.icon!="") {
                ...
                if(pattrs.isClose!=null&&pattrs.isClose=="1"){//是否显示关闭按钮
                	 title.prepend("<a href='#' class='ui-corner-all ui-portlet-event'><span class='ui-icon ui-icon-closethick ui-portlet-close'></span></a>");
                }
            }
            if(pattrs.refreshTime!=null&&pattrs.refreshTime!=""){
            	
            	setInterval(function(){
                	$("#"+pattrs.attrs.id +" .ui-portlet-refresh").click();
                },pattrs.refreshTime*1000);
