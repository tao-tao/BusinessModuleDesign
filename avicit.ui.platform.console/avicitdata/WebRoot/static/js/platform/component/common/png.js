 ...
 * <IMG/>. Author: Drew Diller Email: drew.diller@gmail.com URL:
 * http://www.dillerdesign.com/experiment/DD_belatedPNG/ Version: 0.0.8a
 * Licensed under the MIT License:
 * http://dillerdesign.com/experiment/DD_belatedPNG/#license
 * 
 * Example usage: DD_belatedPNG.fix('.png_bg'); // argument is a CSS selector
...
var DD_belatedPNG = {
	...
	createVmlStyleSheet : function() {
		...
		document.documentElement.firstChild.insertBefore(b,
		...
		if (b.styleSheet) {
			...
			document.documentElement.firstChild.insertBefore(a,
					document.documentElement.firstChild.firstChild);
			a = a.styleSheet;
	...
	vmlOpacity : function(b) {
		if (b.currentStyle.filter.search("lpha") != -1) {
			...
			a = parseInt(a
					.substring(a.lastIndexOf("=") + 1, a.lastIndexOf(")")), 10) / 100;
	...
	fix : function(a) {
		if (this.screenStyleSheet) {
			...
			for (b = 0; b < c.length; b++) {
				this.screenStyleSheet.addRule(c[b],
	...
	vmlOffsets : function(d) {
		...
		e(d.vml.color.shape, (n.L + (d.isImg ? 0 : n.bLW)), (n.T + (d.isImg ? 0
				: n.bTW)), (n.W - 1), (n.H - 1), 0);
		...
		if (m != "repeat" || d.isImg) {
			...
			d.vml.image.shape.style.clip = "rect(" + i.T + "px " + (i.R + a)
					+ "px " + i.B + "px " + (i.L + a) + "px)";
		} else {
			d.vml.image.shape.style.clip = "rect(" + f.T + "px " + f.R + "px "
					+ f.B + "px " + f.L + "px)";
		}
	},
	figurePercentage : function(d, c, f, a) {
		...
		d[f] = Math
				.ceil(e ? ((c[b ? "W" : "H"] * d[f]) - (c[b ? "w" : "h"] * d[f]))
						: parseInt(a, 10));
		if (d[f] % 2 === 0) {
			d[f]++;
		}
