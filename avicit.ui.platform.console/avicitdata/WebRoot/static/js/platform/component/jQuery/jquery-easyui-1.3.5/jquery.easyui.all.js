 ...
 * http://www.gnu.org/licenses/gpl.txt
 * http://www.jeasyui.com/license_commercial.php
 * 刘伟 20140116修改，为了支持icon的定位，linkbutton必须有两个span，图标与方案分开。
 */
(function($){
	...
	function createButton(target) {
		...
		//start
		t.html(
			'<span class="l-btn-left"><span class="'+opts.iconCls+'"></span>' +
...
(function($){
	 ...
	 */
	function createDialog(title, content, buttons, appendIframe){
	...
	$.messager = {
		...
		alert: function(title, msg, icon, fn, appendIframe) {
			...
			var win = createDialog(title,content,buttons,appendIframe);
			return win;
		},
		
		confirm: function(title, msg, fn, appendIframe) {
		...
		prompt: function(title, msg, fn, appendIframe) {
			var content = '<div class="messager-icon messager-question"></div>'
						+ '<div>' + msg + '</div>'
						+ '<br/>'
						+ '<div style="clear:both;"/>'
...
//		}
	...
	function addPanel(container, param, el){
		...
		if (pp.panel('options').split){
			...
			panel.children('div.panel-split');
			var handles = '';
...
(function($){
	...
	function init(target){
		...
		$(target).addClass('numberBox_extend');
		return v;
	}
...
//							val = row[field] || '&nbsp;';
		...
		transfer: function(target, parentId, data){
			...
			for(var i=0; i<rows.length; i++){
				var row = rows[i];
				if (!parentId){
				//modify by lizg treegrid解析树需要 
					if (!row._parentId ||  row._parentId==-1){
						nodes.push(row);
...
//						removeArrayItem(rows,row);
	...
	$.fn.treegrid.defaults = $.extend({}, $.fn.datagrid.defaults, {
		...
		finder:{
		//lizg 虚拟滚动需要
			getTr:function(target, id, type, serno,scroll){
				type = type || 'body';
				serno = serno || 0;
				...
				if (serno == 0){
					...
					if (type == 'body'){
						if(scroll){
							return (serno==1?dc.body1:dc.body2).find('tr[index-no="'+id+'"]');
						}else{
...
(function($){
	function setSize(target, width){
		...
		combo._outerWidth(parseInt(opts.width)+2)._outerHeight(opts.height);
...
//		$('<input type="hidden" class="combo-value">').appendTo(span);
		var panel = $('<div class="combo-panel"><iframe name=\"kill_activex\" frameborder=\"0\" class=\"popup-activex-iframe\"></iframe></div>').appendTo('body');
		panel.panel({
			doSize:false,
			closed:true,
			cls:'combo-p',
...
//		input.attr('autocomplete', 'off');
	...
	function showPanel(target){
		...
		//解决遮挡  gaohailong 
		var activexIframe = panel.siblings(".popup-activex-iframe");
		if(activexIframe.length==0){
			panel.parent().append("<iframe name=\"kill_activex\" frameborder=\"0\" class=\"popup-activex-iframe\"></iframe>");
		}
		if ($.fn.window){
...
//			setValue(target, opts.value);
	...
	$.fn.datetimebox.defaults = $.extend({}, $.fn.datebox.defaults, {
		...
		},{ //add by zl 增加datetimebox清除功能
			text: function(target){return $(target).datetimebox('options').clearText;},
			handler: function(target){
				setValue(target,'');
				$(this).closest('div.combo-panel').panel('close');
			}
