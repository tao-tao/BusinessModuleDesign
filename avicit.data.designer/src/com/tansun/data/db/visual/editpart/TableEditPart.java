...
public class TableEditPart extends AbstractDBEntityEditPart implements
	...
	private ColumnFigure[] createColumnFigure(RootModel root, TableModel table,
			ColumnModel model) {
	...
	public static void openTableEditDialog(EditPartViewer viewer,
		...
		TableEditDialog dialog = new TableEditDialog(viewer.getControl()
	...
	private static class TableEditCommand extends Command {
		...
		public TableEditCommand(TableModel model, String newTableName,
	...
	public void finish(MakeJavaEvent name) {
		...
		if (name.getName().equals(((TableModel) getModel()).getTableName())) {
	...
	class MyCommand extends Command {
		private TableEditPart model;
		private RGB r;
		private MakeJavaEvent event;

		MyCommand(RGB r, TableEditPart model,MakeJavaEvent event) {
			this.model = model;
			this.r = r;
			this.event=event;
		}
		...
		public void execute() {
			TableModel models=((TableModel) model.getModel());
