...
public class RootEditPart extends AbstractDBEditPart {
	...
	public void propertyChange(PropertyChangeEvent evt) {
		...
		if (evt.getPropertyName().equals(RootModel.P_MODE)) {
			...
			for (AbstractDBEntityEditPart part : children) {
				part.refresh();
				@SuppressWarnings("unchecked")
				List<AbstractDBConnectionEditPart> conns = part
	...
	private class RootEditPolicy extends XYLayoutEditPolicy {
		...
		protected Command getCreateCommand(CreateRequest request) {
			...
			AbstractDBEntityModel model = (AbstractDBEntityModel) request
					.getNewObject();
		...
		public void setModel(Object model) {
			...
			if (this.model instanceof TableModel) {
				if (this.model instanceof ViewModel) {
					((TableModel) this.model).setTableName("VIEW_"
							+ (root.getChildren().size() + 1));
					((TableModel) this.model).setLogicalName(DBPlugin
							.getResourceString("label.table")
							+ (root.getChildren().size() + 1));
				} else {
					((TableModel) this.model).setTableName("TABLE_"
							+ (root.getChildren().size() + 1));
					((TableModel) this.model).setLogicalName(DBPlugin
							.getResourceString("label.table")
							+ (root.getChildren().size() + 1));
