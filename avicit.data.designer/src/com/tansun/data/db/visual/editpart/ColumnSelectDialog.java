...
public class ColumnSelectDialog extends Dialog {
	...
	@Override protected Control createDialogArea(Composite parent) {
		...
		viewer.setInput(getColumn(columns));
   ...
   public List<ColumnModel> getColumn(List<ColumnModel> model){
	   List<ColumnModel> colum=new ArrayList<ColumnModel>();
	   for(int i=0;i<model.size();i++){
		   if(!model.get(i).isPrimaryKey()){
			   colum.add(model.get(i));
		   }
	   }
	   return colum;
   }
