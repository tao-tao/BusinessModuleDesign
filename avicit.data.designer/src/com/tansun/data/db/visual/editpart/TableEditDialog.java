...
import com.tansun.data.db.dialect.IIndexType;
import com.tansun.data.db.dialect.IndexType;
import com.tansun.data.db.sqleditor.SQLConfiguration;
import com.tansun.data.db.sqleditor.SQLPartitionScanner;
...
//	private TableViewer indexColumns;
	...
	@Override protected Control createDialogArea(Composite parent) {
		...
		UIUtils.createLabel(group, "dialog.table.editColumn.float");
		txtColumnSizef = new Text(group, SWT.BORDER);
		txtColumnSizef.setLayoutData(UIUtils.createGridDataWithWidth(60));
		txtColumnSizef.addFocusListener(updateColumnListener);
		...
		chkIsPK.addSelectionListener(new SelectionListener() {
	...
	private void updateColumn(){
		if(editColumnIndex != -1 && cmbColumnType.getSelectionIndex() != -1){
			...
			model.setDigits(Integer.valueOf(txtColumnSizef.getText()));
			...
			if(model.getDommain() == null){
				...
				if(!model.getColumnType().getName().toUpperCase().equals("NUMBER")){
					txtColumnSizef.setEnabled(false);
					}else{
						txtColumnSizef.setEnabled(true);
					}
			} else {
				txtColumnSize.setEnabled(false);
				if(!model.getColumnType().getName().toUpperCase().equals("NUMBER")){
	...
	private void tableSelectionChanged(){
		...
		if(index >= 0){
			...
			txtColumnSizef.setText(String.valueOf(model.getDigits()));
	...
	private void updateTableItem(TableItem item, ColumnModel model){
		...
		if(model.getColumnType().supportSize() && model.getSize().length() > 0){
			...
			if(model.getDigits()!=0&&model.getColumnType().getName().equals("NUMBER")){
				sb.append("(").append(model.getSize()).append(",").append(model.getDigits()).append(")");
			}else{
