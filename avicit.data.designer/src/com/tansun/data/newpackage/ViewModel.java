...
public class ViewModel extends TableModel implements ICloneableModel {
	...
	public boolean canSource(AbstractDBConnectionModel conn) {
		if(conn instanceof AnchorModel){
			if(conn.getTarget() != null && conn.getTarget() instanceof ViewModel){
				return false;
			}
		}
		return true;
	...
	public boolean canTarget(AbstractDBConnectionModel conn) {
		if(conn instanceof AnchorModel){
			if(conn.getSource() instanceof ViewModel){
				return false;
			}
		}
		return true;
