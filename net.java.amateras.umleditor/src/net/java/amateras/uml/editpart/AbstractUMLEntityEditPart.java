...
public abstract class AbstractUMLEntityEditPart extends AbstractUMLEditPart
	...
	private class ReconnectTargetCommand extends Command {
		...
		public void execute() {
			oldBendpoints = new ArrayList<ConnectionBendpoint>(
					connection.getBendpoints());
			connection.detachTarget();
			connection.setTarget(target);
			connection.attachTarget();
			for (Iterator<ConnectionBendpoint> iterator = oldBendpoints
					.iterator(); iterator.hasNext();) {
				ConnectionBendpoint point = (ConnectionBendpoint) iterator
						.next();
		...
		public void undo() {
			...
			for (int i = 0; i < oldBendpoints.size(); i++) {
				connection.addBendpoint(i,
						(ConnectionBendpoint) oldBendpoints.get(i));
			}
		}
	}
