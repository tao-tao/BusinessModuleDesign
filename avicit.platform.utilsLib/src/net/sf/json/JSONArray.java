...
public final class JSONArray extends AbstractJSON implements JSON, List, Comparable {
	 ...
	 * found.<br/> Contributed by [Matt Small @ WaveMaker].
	 */
	...
	/**
	 * Returns a List or a Set taking generics into account.<br/> Contributed
	 * by [Matt Small @ WaveMaker].
	 */
	...
	/*
	 * public static Collection toCollection( JSONArray jsonArray, JsonConfig
	 * jsonConfig ) { Collection collection = null; Class collectionType =
	 * jsonConfig.getCollectionType(); Class enclosedType =
	 * jsonConfig.getEnclosedType();
	 * 
	 * if( collectionType.isInterface() ){ if( collectionType.equals( List.class ) ){
	 * collection = new ArrayList(); }else if( collectionType.equals( Set.class ) ){
	 * collection = new HashSet(); }else{ throw new JSONException( "unknown
	 * interface: " + collectionType ); } }else{ try{ collection = (Collection)
	 * collectionType.newInstance(); }catch( InstantiationException e ){ throw
	 * new JSONException( e ); }catch( IllegalAccessException e ){ throw new
	 * JSONException( e ); } }
	 * 
	 * Class objectClass = jsonConfig.getRootClass(); Map classMap =
	 * jsonConfig.getClassMap();
	 * 
	 * int size = jsonArray.size(); for( int i = 0; i < size; i++ ){ Object
	 * value = jsonArray.get( i ); Class enclosedTypeE = enclosedType;
	 * 
	 * if( null == enclosedTypeE ){ enclosedTypeE = value.getClass(); }
	 * 
	 * if( JSONUtils.isNull( value ) ){ collection.add( null ); }else{ if(
	 * JSONArray.class.isAssignableFrom( value.getClass() ) ){ //throw new
	 * RuntimeException( "can't have nested collections" ); collection.add(
	 * toCollection( (JSONArray) value, jsonConfig ) ); }else if(
	 * String.class.isAssignableFrom( enclosedTypeE ) ||
	 * Boolean.class.isAssignableFrom( enclosedTypeE ) || JSONUtils.isNumber(
	 * enclosedTypeE ) || Character.class.isAssignableFrom( enclosedTypeE ) ||
	 * JSONFunction.class.isAssignableFrom( enclosedTypeE ) ){
	 * 
	 * if( !value.getClass() .isAssignableFrom( enclosedTypeE ) ){ throw new
	 * JSONException( "can't assign value " + value + " of type " +
	 * value.getClass() + " to Collection of type " + enclosedTypeE ); }
	 * collection.add( value ); }else{ try{ if( JSON.class.isAssignableFrom(
	 * enclosedTypeE ) ){ ret.add( JSONObject.toBean( (JSONObject) value ) );
	 * }else{ Object newRoot = enclosedTypeE.newInstance(); ret.add(
	 * JSONObject.toBean( (JSONObject) value, newRoot, jsonConfig ) ); } }catch(
	 * JSONException jsone ){ throw jsone; }catch( Exception e ){ throw new
	 * JSONException( e ); } if( objectClass != null ){ JsonConfig jsc =
	 * jsonConfig.copy(); jsc.setRootClass( objectClass ); jsc.setClassMap(
	 * classMap ); collection.add( JSONObject.toBean( (JSONObject) value, jsc ) );
	 * }else{ collection.add( JSONObject.toBean( (JSONObject) value ) ); } } } }
	 * 
	 * return collection; }
	 */
